...
'insertFields'  => "array(<?php echo $_fields_arr; ?>)",                       // 添加时允许接收的表单中的字段（要生成的模型文件中的代码）
'updateFields'  => "array('<?php echo $_pk; ?>',<?php echo $_fields_arr; ?>)", // 修改时允许接收的表单中的字段
'validate'      => "
<?php foreach ($_tableFields as $k => $v):
    ...
    if (strpos($v['Type'], 'varchar') === 0): ?>
        array('<?php echo $v['Field']; ?>', '1,<?php $_s1 = str_replace(array('varchar(', ')'), array('', ''), $v['Type']);
        echo $_s1; ?>', '<?php echo $v['Comment']; ?>的值最长不能超过 <?php echo $_s1; ?> 个字符！', <?php echo $_chkTime; ?>, 'length', 3),
    <?php endif;
    if (strpos($v['Type'], 'char') === 0): ?>
        array('<?php echo $v['Field']; ?>', '1,<?php $_s1 = str_replace(array('char(', ')'), array('', ''), $v['Type']);
        echo $_s1; ?>', '<?php echo $v['Comment']; ?>的值最长不能超过 <?php echo $_s1; ?> 个字符！', <?php echo $_chkTime; ?>, 'length', 3),
    <?php endif;
endforeach; ?>
<?php foreach ($_tableFields as $k => $v):
...
<?php foreach ($_tableFields as $k => $v):
    ...
    '<?php echo $v['Field']; ?>' => array(
        ...
        'save_fields' => array('<?php echo $v['Field']; ?>', 'sm_<?php echo $v['Field']; ?>', 'mid_<?php echo $v['Field']; ?>', 'big_<?php echo $v['Field']; ?>', 'mbig_<?php echo $v['Field']; ?>'),
